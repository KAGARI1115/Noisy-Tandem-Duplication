...
% This function accounts for the case when ell+1<=k<=2ell
...
function A = create_k_mer_A_matrix_noisydup_small_k_new(k,alph,ell,d)
M = alph^k;
A = zeros(M,M);
...
szE = size(Epos);
for j = 1:szE(1)
    A_j = create_k_mer_A_matrix_small_k_noise_fix(k,alph,ell,Epos(j,:));
    A = A + A_j;
end
A = A/szE(1);
end

function A = create_k_mer_A_matrix_small_k_noise_fix(k,alph,ell,evec)
...
for vnum = 1:M
    ...
    for b = 1:k-ell-1
        ...
        epos = find(eb);
        nume = length(epos);
        Vtem = Hamball_pos(v(k-b+1-ell:k-ell),alph,epos);
        for j = 1:(alph-1)^nume
            vtail = Vtem(j,:);
            u = [v(1:k-b),vtail];
            ui = seq2num(u,alph);
    ...
    for b = k-ell:ell
        ...
        Y = Hamball_pos(v(ell-(k-b)+1:b),alph,epos2);
        for i = 1:(alph-1)^nume1
            for j = 1:(alph-1)^nume2
                x = X(i,:);
                y = Y(j,:);
                u = [v(ell-(k-b)+1:ell),x,y];
                ui = seq2num(u,alph);
